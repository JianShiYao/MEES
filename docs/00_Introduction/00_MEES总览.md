...
- 完成度：60%
- 当前门禁：v0.6.0 已通过 V6-G6 `Go`（模拟审计，候选 `9b3cde5` GitHub CI 绿）——六类 Agent 只读运行时（不批准门禁）、Obsidian O0–O2 只读与受控本地写入（V6-G3/G4 已签）成内部基线。证据为工具 `P` + 数据 `S`；Agent 不批准门禁、产品 `No-Go`；不构成真实独立评估、标准符合性、认证或能力等级；REST 写入 Obsidian 为后续能力。下一阶段 v1.0 组织级基线
