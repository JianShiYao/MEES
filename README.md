...
- 版本：v1.0.0（内部整合基线，模拟审计签署）
- 阶段：v1.0.0 已发布（V1-G0–G6 已批准，非组织级落地）
- 状态：V1-G6 已 `Go`（模拟审计，候选 `0cd2569` GitHub CI 绿）。v0.1–v0.6 整合为干净、可评审、可裁剪的内部基线：语料一致、编号债关闭、单一权威边界与证据声明、统一遗留台账（严重/主要=0）。证据仍为 `D`/`S`（模拟）；产品 `No-Go`；不构成真实独立评估、标准符合性、认证或能力等级；非真实组织级 1.0 落地
