...
| `16_Agents/` | Agent 能力、使用边界、协议和治理文档；当前已建立 v0.6 协作总览。 | 解释根目录 `agents/` 中 Agent 资产的设计与使用方法。 |
| `17_Tools/` | 工具链选型、接入、配置和操作说明；当前已建立 Obsidian/MCP 分层接入策略。 | 解释 `scripts/`、DevOps 平台及外部工程工具如何支撑各过程。 |
...
- 版本：v0.6.0（Agent 与 Obsidian 集成内部基线，模拟审计签署）
- 阶段：v0.6.0 已发布；v1.0 内部整合基线建设中（V1-G0/G1/G2 已批准，非组织级落地）
- 状态：V6-G6 已 `Go`（模拟审计，候选 `9b3cde5` GitHub CI 绿）。六类 Agent 只读运行时（不批准门禁）、Obsidian O0–O2 只读 + 受控本地写入（V6-G3/G4 已签）已成内部基线。证据为工具 `P` + 数据 `S`；Agent 不批准门禁、产品 `No-Go`；不构成真实独立评估、标准符合性、认证或能力等级；REST 写入 Obsidian 为后续能力
- 边界与证据：统一定义见 `docs/00_Introduction/09_边界与证据声明.md`
- 日期：2026-07-16
